...
class Blocksy_Header_Builder_Render extends Blocksy_Builder_Render {
	...
	private function render_start_placement($row, $args = []) {
		...
		if (count($placement['items']) > 0) {
			$primary_items_collection = $this->render_items_collection(
				$placement['items'],
				[
					'device' => $args['device']
				]
			...
			if (! empty($primary_items_collection)) {
				$primary_output = blocksy_html_tag(
					'div',
					[
						'data-items' => 'primary'
					],
					$primary_items_collection
				);
			}
		}
		...
		if (
			...
			$secondary_items_collection = $this->render_items_collection(
				$start_secondary['items']
			);

			if (! empty($secondary_items_collection)) {
				$secondary_output = blocksy_html_tag(
					'div',
					[
						'data-items' => 'secondary'
					],
					$secondary_items_collection
				);
			}
		}
